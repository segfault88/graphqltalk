...
func (l *MovieLoader) Load(key int) (Movie, error) {
...
func (l *MovieLoader) LoadThunk(key int) func() (Movie, error) {
	l.mu.Lock()
...
func (l *MovieLoader) LoadAll(keys []int) ([]Movie, []error) {
	results := make([]func() (Movie, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	movies := make([]Movie, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *MovieLoader) LoadAllThunk(keys []int) func() ([]Movie, []error) {
	results := make([]func() (Movie, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *MovieLoader) Prime(key int, value Movie) bool {
	l.mu.Lock()
	var found bool
...
func (l *MovieLoader) unsafeSet(key int, value Movie) {
	if l.cache == nil {
		l.cache = map[int]Movie{}
	}
	l.cache[key] = value
}
